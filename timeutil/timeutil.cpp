...
std::string time2string(const tm& brokenTime, const char* fmt, const long* usec,
                        const char* usecSep) {
    char buffer[64];
...
bool parseDateTime(const char* str, timeval& t) {
    ...
    long subSec;
    int filled = sscanf(str, "%04d%*c%02d%*c%02d%*c%02d%*c%02d%*c%02d%*c%06ld", &dateTime.tm_year,
                        &dateTime.tm_mon, &dateTime.tm_mday, &dateTime.tm_hour, &dateTime.tm_min,
                        &dateTime.tm_sec, &subSec);
    if (filled < 6)
        return false;
